...
async function buildTemplate({
  ...
  dataSheet.getRange(`A1:${endColumn}${examples.length + 1}`).values = [
    headers,
    ...examples,
  ];
  dataSheet.getRange(`A1:${endColumn}1`).format = {
    fill: "#172D72",
  ...
  dataSheet.getRange(`A2:${endColumn}${examples.length + 1}`).format = {
    fill: "#F7F9FC",
    font: { color: "#172033" },
    rowHeight: 24,
    verticalAlignment: "center",
  ...
  if (fileName.includes("KENDARAAN")) {
    dataSheet.getRange("D2:D500").format.numberFormat = "#,##0";
    dataSheet.getRange("E2:E500").dataValidation = {
      rule: { type: "list", values: ["INBOUND", "OUTBOUND"] },
    };
    dataSheet.getRange("F2:F500").dataValidation = {
      rule: { type: "list", values: ["AKTIF", "NONAKTIF"] },
    };
  } else {
    dataSheet.getRange("F2:F500").dataValidation = {
      rule: { type: "list", values: ["AKTIF", "NONAKTIF"] },
  ...
  const table = dataSheet.tables.add(
    `A1:${endColumn}${examples.length + 1}`,
    true,
    fileName.includes("KENDARAAN") ? "TemplateKendaraan" : "TemplateDriver",
  );
  ...
  const instructionRows = instructions.map((text, index) => [
    `${index + 1}.`,
    text,
    null,
    null,
  ...
  helpSheet.getRange(`A4:F${instructionRows.length + 3}`).values =
    instructionRows;
  helpSheet.getRange(`A4:A${instructionRows.length + 3}`).format = {
    font: { bold: true, color: "#E31E24" },
    horizontalAlignment: "center",
  };
  helpSheet.getRange(`B4:F${instructionRows.length + 3}`).merge(true);
  helpSheet.getRange(`B4:F${instructionRows.length + 3}`).format = {
    wrapText: true,
    font: { color: "#334155" },
    rowHeight: 30,
    verticalAlignment: "center",
  ...
  const check = await workbook.inspect({
    kind: "table",
    range: `Data!A1:${endColumn}${examples.length + 1}`,
  ...
  await fs.rm(path.join(outputDir, `${fileName}.xlsx.inspect.ndjson`), {
    force: true,
  });
}
...
await buildTemplate({
  ...
  instructions: [
    ...
    "Unit Aset isi INBOUND atau OUTBOUND, atau nama unit lain yang sudah dibuat di Pengaturan Akun.",
...
await buildTemplate({
  ...
  widths: [24, 18, 20, 18, 22, 16, 32],
  instructions: [
    "Isi data pada sheet Data. Jangan mengubah nama kolom pada baris pertama.",
    "Nama Driver, ID Login, Nomor HP, dan Password wajib diisi. ID Login serta Nomor HP tidak boleh duplikat.",
