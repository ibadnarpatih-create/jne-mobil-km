...
import { Download, ExternalLink, Eye, MapPin, Search, ShieldCheck, Trash2, X } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Input, Label, Select } from "@/components/ui/field";
import { deleteFuelTransaction, loadFuelTransactions, reviewFuelTransaction, type FuelTransactionRecord } from "@/lib/fuel/admin";
...
export function FuelTransactionsPanel({ mode }: { mode: "transactions" | "validation" | "report" }) {
  ...
  return <div className="space-y-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-2xl font-extrabold">{title}</h2><p className="mt-1 text-sm text-slate-500">{mode === "validation" ? `${filtered.length} transaksi menunggu pemeriksaan` : "Data pengisian terhubung dengan kendaraan dan odometer."}</p></div>{mode === "report" && <div className="flex gap-2"><Button variant="outline" onClick={() => exportCsv(filtered, users, vehicles, master)}><Download className="h-4 w-4" /> CSV</Button><Button onClick={() => exportExcel(filtered, users, vehicles, master)}><Download className="h-4 w-4" /> Excel</Button></div>}</div>
    ...
    {selected && <TransactionDetail item={selected} users={users} vehicles={vehicles} master={master} allowReview={mode === "validation"} onClose={closeDetail} onReviewed={async () => { closeDetail(); await reload(); }} onDeleted={async () => { closeDetail(); await reload(); }} />}
  </div>;
}
...
function TransactionDetail({ item, users, vehicles, master, allowReview, onClose, onReviewed, onDeleted }: { item: FuelTransactionRecord; users: ReturnType<typeof useDemoStore>["users"]; vehicles: ReturnType<typeof useDemoStore>["vehicles"]; master: FuelMasterData; allowReview: boolean; onClose: () => void; onReviewed: () => Promise<void>; onDeleted: () => Promise<void> }) {
  ...
  const remove = async () => { if (!window.confirm("Hapus transaksi BBM ini? Data dan foto terkait tidak dapat dipulihkan.")) return; setBusy(true); setError(""); try { await deleteFuelTransaction(item.id); await onDeleted(); } catch (cause) { setError(cause instanceof Error ? cause.message : "Transaksi belum berhasil dihapus."); } finally { setBusy(false); } };
