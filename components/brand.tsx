...
export function Brand({ compact = false, className }: { compact?: boolean; className?: string }) {
  return (
    <div className={cn("flex items-center gap-3", className)}>
      ...
      <div>
        <p className={cn("font-extrabold leading-tight text-jne-blue", compact ? "text-base" : "text-xl")}>Movetra</p>
        {!compact && <p className="text-xs text-slate-500">Move Smarter, Work Better</p>}
      </div>
    </div>
  );
}
